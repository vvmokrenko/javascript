...
const snake = {
  ...
  maxX: null, //Максимальная позиция змейки на карте по оси X.
  maxY: null, //Максимальная позиция змейки на карте по оси Y.
  ...
  getNextStepHeadPoint() {
    ...
    switch(this.direction) { //Убираем  границы поля
      case 'up':
...
const game = {
  ...
  reset() {
    this.stop();
    this.score.drop(); //Обнуляем счетчик
  ...
  tickHandler() {
    ...
    if (this.isOnFood()) {
      this.snake.growUp(); 
      this.score.increment(); //Увеличиваем счет
      this.food.setCoordinates(this.getRandomFreeCoordinates());
